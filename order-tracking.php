 ...
 * Sweets Website
 * =============================================================
 * File: order-tracking.php
 * Description: Premium Order Tracking/Status Page (High-Fidelity)
 * =============================================================
 */
...
$trackingHistory = $orderId ? $orderService->getOrderTracking((int)$orderId) : [];

// Fallback for demo if no order found
if (!$order) {
...
$taxAmount = (float)($order['tax_amount'] ?? ($totalAmount * 0.18)); // Use DB tax or 18% fallback
...
$estimatedDelivery = !empty($order['estimated_delivery_date']) 
    ? date('d M Y', strtotime($order['estimated_delivery_date'] . ' - 2 days')) . ' - ' . date('d M Y', strtotime($order['estimated_delivery_date']))
    : date('d M Y', strtotime(($order['created_at'] ?? 'now') . ' + 3 days')) . ' - ' . date('d M Y', strtotime(($order['created_at'] ?? 'now') . ' + 5 days'));
...
// If status is 'paid', it's basically 'processing' in our flow
...
// Ensure logic for higher states
if ($status === 'shipped') $currentStepIndex = 2;
if ($status === 'out_for_delivery') $currentStepIndex = 3;
if ($status === 'delivered') $currentStepIndex = 4;

// Build actual milestone completion timestamps from history
$statusTimes = [];
foreach ($trackingHistory as $historyItem) {
    $histStatus = strtolower($historyItem['status']);
    if ($histStatus === 'placed' || $histStatus === 'pending') {
        $statusTimes['pending'] = $historyItem['created_at'];
    } elseif ($histStatus === 'packed' || $histStatus === 'processing') {
        $statusTimes['processing'] = $historyItem['created_at'];
    } elseif ($histStatus === 'shipped') {
        $statusTimes['shipped'] = $historyItem['created_at'];
    } elseif ($histStatus === 'out_for_delivery') {
        $statusTimes['out_for_delivery'] = $historyItem['created_at'];
    } elseif ($histStatus === 'delivered') {
        $statusTimes['delivered'] = $historyItem['created_at'];
    }
}
    ...
    <div class="container">
        ...
        <nav class="ot-breadcrumb mb-4" aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="my-orders.php">Orders</a></li>
            ...
            <div class="col-lg-8">
                <div class="d-flex align-items-center gap-2 mb-2">
                    ...
                    <?php if ($status === 'cancelled' || $status === 'failed'): ?>
                        <span class="ot-badge bg-danger-subtle text-danger" style="background-color: rgba(220, 53, 69, 0.1); color: #dc3545;"><?php echo ucfirst($status); ?></span>
                    <?php else: ?>
        ...
        <!-- Cancelled or Failed Order Alert -->
        <?php if ($status === 'cancelled'): ?>
            <div class="alert alert-danger d-flex align-items-center gap-3 p-4 rounded-3 border-0 shadow-sm mb-4 sc-reveal" style="background-color: rgba(220, 53, 69, 0.05); border: 1px solid rgba(220, 53, 69, 0.15) !important;">
                <i class="bi bi-x-circle-fill text-danger fs-3"></i>
                <div>
                    <h5 class="alert-heading fw-bold mb-1" style="color: #842029;">Order Cancelled</h5>
                    <p class="mb-0 small text-danger-emphasis">This order has been cancelled. If this was a mistake or you have questions, please contact our support team.</p>
                </div>
            </div>
        <?php elseif ($status === 'failed'): ?>
            <div class="alert alert-danger d-flex align-items-center gap-3 p-4 rounded-3 border-0 shadow-sm mb-4 sc-reveal" style="background-color: rgba(220, 53, 69, 0.05); border: 1px solid rgba(220, 53, 69, 0.15) !important;">
                ...
                <div>
                    <h5 class="alert-heading fw-bold mb-1" style="color: #842029;">Payment Failed</h5>
                    <p class="mb-0 small text-danger-emphasis">The payment transaction for this order failed. Please place a new order or contact support if the amount was debited.</p>
                </div>
            </div>
        <?php endif; ?>
                                ...
                                <?php foreach ($steps as $index => $step): ?>
                                    ...
                                    <div class="ot-step <?php echo $isCompleted ? 'is-completed' : ''; ?> <?php echo $isActive ? 'is-active' : ''; ?>">
                                        ...
                                        <div class="ot-step-date small text-muted">
                                            <?php 
                                                $stepTime = $statusTimes[$step['key']] ?? null;
                                                if ($step['key'] === 'pending' && !$stepTime) {
                                                    $stepTime = $order['created_at'];
                                                }
                                                echo $isCompleted && $stepTime ? date('d M Y, h:i A', strtotime($stepTime)) : '-'; 
                                            ?>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                ...
                <div class="ot-card shadow-sm border-0" style="background: #fffdfb;">
                    <div class="ot-card-body p-4">
                        ...
                        <div class="d-grid gap-2">
                            ...
                            <button type="button" class="btn btn-outline-secondary text-start p-2" data-bs-toggle="modal" data-bs-target="#trackingModal">
...
<div class="modal fade" id="trackingModal" tabindex="-1" aria-labelledby="trackingModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 16px; overflow: hidden;">
            <div class="modal-header border-bottom-0 pb-0" style="background: #fdfaf6;">
                <h5 class="modal-title fw-bold" id="trackingModalLabel" style="color: #6C2C23;">
                    <i class="bi bi-geo-alt-fill me-2 text-danger"></i>Tracking History
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body pt-4" style="background: #fdfaf6;">
                <?php if (empty($trackingHistory)): ?>
                    <div class="text-center py-4">
                        <div class="text-muted mb-3"><i class="bi bi-info-circle" style="font-size: 2rem;"></i></div>
                        <h6>No tracking updates yet</h6>
                        <p class="text-muted small">Tracking information will appear here once your order is processed and shipped.</p>
                    </div>
                <?php else: ?>
                    <div class="vertical-timeline position-relative ps-4 py-2">
                        <!-- Timeline line -->
                        <div class="position-absolute h-100" style="left: 20px; top: 0; width: 2px; background: #e8dcd0; z-index: 1;"></div>
                        
                        <?php foreach($trackingHistory as $index => $historyItem): 
                            $isFirst = ($index === 0);
                            $dateStr = date('D, d M Y', strtotime($historyItem['created_at']));
                            $timeStr = date('h:i A', strtotime($historyItem['created_at']));
                            $statusLabel = strtoupper($historyItem['status']);
                            $location = !empty($historyItem['location']) ? $historyItem['location'] : '';
                            $desc = !empty($historyItem['description']) ? $historyItem['description'] : '';
                            
                            // Determine dot color
                            $dotColor = $isFirst ? '#28a745' : '#c4b5a3'; // Green for latest, gray for older
                            if ($statusLabel === 'DELIVERED') $dotColor = '#28a745';
                            if ($statusLabel === 'CANCELLED' || $statusLabel === 'FAILED') $dotColor = '#dc3545';
                        ?>
                        <div class="timeline-item position-relative mb-4" style="z-index: 2;">
                            <!-- Timeline Dot -->
                            <div class="position-absolute rounded-circle shadow-sm d-flex align-items-center justify-content-center" 
                                 style="left: -26px; top: 2px; width: 14px; height: 14px; background: <?php echo $dotColor; ?>; border: 2px solid #fdfaf6;">
                            </div>
                            
                            <!-- Timeline Content -->
                            <div class="d-flex justify-content-between">
                                <div>
                                    <h6 class="mb-1 fw-bold" style="color: <?php echo $isFirst ? '#2e1a0e' : '#6a5e55'; ?>;">
                                        <?php echo htmlspecialchars($statusLabel); ?>
                                    </h6>
                                    <?php if($desc): ?>
                                        <p class="mb-1 small" style="color: <?php echo $isFirst ? '#5a463a' : '#8a7d74'; ?>;">
                                            <?php echo htmlspecialchars($desc); ?>
                                        </p>
                                    <?php endif; ?>
                                    <?php if($location): ?>
                                        <p class="mb-0 text-muted" style="font-size: 0.75rem;">
                                            <i class="bi bi-pin-map-fill me-1"></i><?php echo htmlspecialchars($location); ?>
                                        </p>
                                    <?php endif; ?>
                                </div>
                                <div class="text-end ms-3">
                                    <div class="small fw-semibold" style="color: #6C2C23;"><?php echo $dateStr; ?></div>
                                    <div class="text-muted" style="font-size: 0.75rem;"><?php echo $timeStr; ?></div>
                                </div>
                            </div>
                        </div>
                        <?php endforeach; ?>
            ...
            <div class="modal-footer border-top-0 pt-0" style="background: #fdfaf6;">
...
<style>
/* Optional custom CSS for Modal scrollbar if needed */
